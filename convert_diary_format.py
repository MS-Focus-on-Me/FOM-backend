...
async def writer_workflow(task_prompt: str, reference: str) -> str:
    ...
    요청 작업:
        1. 시간순으로 내용 작성
        2. 날짜는 맨 위에 'YYYY년 MM월 DD일'로 한 번만 표시
        3. 시간은 표시하지 말고 자연스러운 흐름으로 연결
        4. 사용자의 문체는 {references} 와 같게 한다.
        5. 중복되거나 유사한 내용은 자연스럽게 통합
        6. 출력은 하나의 완성된 일기처럼 구성
    '''
